...
/// <summary>
/// 隐身模式服务 - 为 ChromeDriver 提供反检测和隐身保护
/// </summary>
internal static class StealthService
{
    ...
    /// </summary>
    /// <param name="chromeOptions">Chrome 选项，为空则使用默认选项</param>
    /// <param name="instanceSettings">隐身设置，为空则使用默认设置</param>
    /// <returns>配置后的 ChromeDriver 实例</returns>
    public static ChromeDriver ApplyStealth(ChromeOptions? chromeOptions, StealthInstanceSettings? instanceSettings)
    {
        instanceSettings ??= new StealthInstanceSettings();
        chromeOptions ??= new ChromeOptions();

        ChromeDriver driver = string.IsNullOrWhiteSpace(instanceSettings.ChromeDriverPath)
